...
read-only inside the container, which is a sensible default for serving static files.

`site/index.html` is committed in its **Version 1** state, so the demo can be run again from a
fresh clone. The `sed` above is what flips it to Version 2, and `git checkout site/index.html`
puts it back.
