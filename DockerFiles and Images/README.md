...
# Hello World from a go1.23.12 binary running in container 27a0702f9e66
...
| Image | Size |
|---|---|
| `golang:1.23-alpine` (build stage base) | ~365 MB, published size |
| `hello-multistage` (final image) | 6.98 MB |

The final image is about 2% of the build image. Everything in it is the one binary.

The build stage base does not show up in `docker images`, which is why the screenshot below
lists only the final image. The `Dockerfile` pins `golang:1.23-alpine` by digest, so BuildKit
pulls it straight into its own build cache instead of registering it as a local image; the
build log shows the layers being fetched (70.70 MB, 284.71 kB, 4.13 MB and so on). Running
`docker pull golang:1.23-alpine` would list it, but nothing in the build needs that.
